...
#### Place for Yos to start playing with data

```{r how does TLP differ}
boxplot(TLP~T.S.L, fullpvs)
...
ggplot(fullspp, aes(x=log(SWC, base=10),y=log(sat.gwat.m2,base=10), col=Biome)) +  xlab("log10 Sat g water per g dry mass\nMass Basis") + ylab("log10 Sat g water per m2\nArea Basis") + geom_smooth(method="lm", se=F) + #geom_point(data=fullpvs) + 
...
palette(mypal)
par(mar=c(6,6,2,1))
plot(sat.gwat.m2~SWC, fullpvs, pch=16, col="grey", log="xy", ylab="Sat g water per m2\nArea Basis", xlab="Sat g Water per g dry mass\nMass Basis")
palette(mypal)
...
boxplot(CFT.g.m2~T.S.L, fullspp)
boxplot(water_loss.m2~T.S.L, fullspp)
ggplot(fullspp, aes(x=log(water_loss.m2, 10), y=log(CFT.g.m2,10), col=T.S.L)) + geom_point() + geom_smooth(method="lm")+ theme_classic() + ylab("CFT (g m-2 MPa-1)") + xlab("TWL (g m-2)")
#plot(CFT.g.m2~water_loss.m2, fullspp, col=factor(Ever.Decid), log="xy", pch=16, ylab="CFT (g m-2 MPa-1)", xlab="TWL (g m-2)")
#legend("topleft",c("fern","herb","woody"), pch=16, col=c(1,2,3))
```
