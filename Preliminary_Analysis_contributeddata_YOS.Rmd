...
\- To Double Check Danielle Ulrich - apoplastic fractions \>1,

\- German Vargas - Species with crazy totRWCtlp and other values,

\- Chris M. Smith-Martin - CFT calculation off.
...
### Data from Bartlett Meta-analyses

We'll bring in Megan's published meta-analyses from Bartlett et al. 2012 and Bartlett et al. 2014. However, these only have RWCtlp and sometimes LMA, and we need SWC (or LDMC) to transform these curves into absolute gH2O per mass or area. So I pulled all the LMA and LDMC data from TRY to infill with species average data where I can.
...
Combining all things into master dataset, we don't get all the metadata we'd like for all the studies, but we increase the \# spp and geographic spread - Let's analyze the relationship between parameters on the full dataset, and then we can explore specific patterns in depth in the contributed dataset with covariates that we don't have for the full dataset
...
-   Still missing 68 families somehow, but will go back and fix those - part of this is that none of Megans Chinese spp have families yet after updating...
...
data frame explainer pvs.master: original fullpvs: whole dataset, contributed and pulled from two meta-analyses, lost some metadata from the pvs.master

##### Yos analysis

Analyzed and ordered based on the factors to consider after looking at different relationships with and within variables.
...
boxplot(SWC~T.S.L, fullpvs, xlab = "T.S.L", ylab = "SWC", main = "Saturated Water Content for Tree Shrub Liana")



```

*TLP median for TSL is around -2 except for bamboo, with the greatest variance & range for trees. Trees possibly have adaptive qualities that allow for TLP to be much lower. SWC shows similar medians, possibly relative to TLP for the purpose of clearly identifying the water content relationship to TLP.*

## Saturated Water Content: How much water is there per dry mass ('SWC') vs m2 leaf area ('SWA')? SWA is the area saturated, closer to identifying wilt through satellite

ANSWER

Biomes are considered next, for a closer relationship, since TSL did not show an average median throughout (other graphs area at the end to look over different variables and their relationship to TSL).
...
*SWC does not have a consistent median across biomes but SWA does. Between the two, SWA is preffered but at the end of the day grams of water could vary from the grams of dry mass with and without rehydration (more on that in HERE). TLP area calculation has an average median too, and RWC\@TLP is not looking too bad as well except for tropical moist.*
...
## Angiosperms and Gymnosperms: How much do biomes differ when looking at angio and gymno side by side for different variables? This could have a different impact on how well water content is portrayed from above.

Next, accounting for the differences between factors that can be analyzed separately.
...
*SWA is much more predictable from SWC without gymnosperms. In comparison to capacitance and total water loss, gymnosperms are much more predictable but angiosperms are not.*

\
black points = species means, colored points = all observations\
...
### Angiosperms only: How is TWL accomplished when looking at RWC\@TLP?
...
*(black line in plot is spp with totRWCtlp \< 80% and blue is spp with totRWCtlp\>80%)*
...
### Further into variable relationships (log-log relationships): Is TWL do to high capacitance (CFT, g water per m2 leaf area per MPa water potential change)? Or to low RWC \@ TLP (i.e. the ability to desiccate your leaves)?
...
Starting with SWC and SWA then looking at the other variables, biomes are considered for the relationships possible. First, are capacitance (CFT) and RWCtlp related? (i.e. could it be both?)

```{r continued SWC on mass vs area basis}
ggplot(fullspp, aes(x=log(SWC, base=10),y=log(sat.gwat.m2,base=10), col=Biome)) +  xlab("log10 - SWC - g water per g dry mass\nMass Basis") + ylab("log10 - SWA - g water per m2\nArea Basis") + geom_smooth(method="lm", se=F) + 
...
All biomes show a positive relationship which is relatively good but can account for unpredictable TLP. A perfect relationship can be seen with TLP area and SWA, which is not the same for TLP and SWC for comparison (other graphs shwoing a negative relationship are in the end).
...
## Decomposing Variance: How much and why do all of these variables vary globally?
...
*TWL, CFT, LMA and SWA are WAY more variable than TLP and RWCtlp* Depending on what metric you look at (xfold.raw is x-fold variation in all data, so prone to outliers, xfold-90 is just the variation from the 5ht -95th quantiles, CV assumes a normal distribution which many of these aren't), either TWL or LMA are the most variable metrics.
...
red = btw families, dark blue = btw genera w/in family, blue = btw spp w/in genus, light blue = within species
...
#### Removing Picea Mariana: Massive Outlier  Look at some of the same attributes per biome and leaf habit
...
Fern, Herb, and Woody plants have similar means for a few variables, but woody plants have a greater variance. Other graphs (total [RWC\@TLP](mailto:RWC@TLP){.email}, capacitance, and SWA) worth looking at are at the end.
...
-Herbaceous and woody plants have fairly similar TWL (woody gets lower as TWL increases), but herbs generally have higher CFT.
...
#legend("topleft",c(""), pch=16, col=c(1,2,3))
...
## PV curves we can 'see' from RS: This is the actual relationship between water potential and absolute g water content (per m2 leaf area). This assumes identical LAI or the viewing of a single leaf.
...
## Figures for AGU: The Methodological elephant in the room... Rehydration. Reconstructing PV curves (i.e. in RWC space)
...
# Stream of Consciousness : Other plots less thought through... SWC, LMA and TLP test of some previously used correlations
...
Can we predict a leaf's characteristics from other characteristics of it? (when those characteristics aren't mathematically related) - Not really. - The previously hypothesized correlation between TLP and SWC/LDMC isn't great

#### Expanded biplots: Moving towards a fishing expedition
...
For some reason Picea mariana is way high for both of these, - I can't tell why. It's not extreme. It's got a pretty high (2nd highest) saturated water content per m2 but also a high tlp.gwat.m2 pretty high LMA, particularly for its SWC?

#####\* A few Quick takeaways: \###### 1) total water to lose is correlated with the capacitance 2) but not correlated with TLP (the end point) 3) total water to loss is only vaguely correlated with SWC 4) total water to loss is somewhat (but not crazy strongly) correlated with LMA 5) total water to loss is strongly nevatively correlated with totRWCtlp
...
Total water content (mass basis) not strongly linked to Capacitance or Tot Water Loss (area basis) But Total Water Content (area basis) is more related to TWL than
...
## Other Graphs: Stored all other graphs, already looked at, in order presented above

TLP - means of T.S.L.

```{r}
boxplot(tlp.gwat.m2~T.S.L, fullpvs, xlab = "T.S.L", ylab = "TLP g/m2", main = "Turgor Loss Point in g/m2 for Tree Shrub Liana")

boxplot(sat.gwat.m2~T.S.L, fullpvs, xlab = "T.S.L", ylab = "SWA", main = "Saturated Water Area for Tree Shrub Liana")
...
Variables for Total [RWC\@TLP](mailto:RWC@TLP){.email}
